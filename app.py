...
if uploaded_files and start_date <= end_date:
    ...
    if st.button("🚀 开始处理并合并数据"):
        ...
        if data_frames:
            ...
            # 计算总和并存入 session_state
            st.session_state.total_clicks = int(final_combined_df['Clicks'].sum())
            st.session_state.total_orders = int(final_combined_df['Orders'].sum())
            st.session_state.total_sales = float(final_combined_df['Sales'].sum())
            # ===============================
